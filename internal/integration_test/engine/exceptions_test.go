...
// testEHExnrefSetInTryBodySurvivesCollection sets an exnref local inside a try body that then
// throws. The handler reads the local, so the exnref must survive the collection the throw
...
func testEHExnrefSetInTryBodySurvivesCollection(t *testing.T, cfg wazero.RuntimeConfig) {
	...
	i32, exnref := wasm.ValueTypeI32, wasm.ValueTypeExnref
	const (
		thrower = iota
		makeExn
		makeDeep
	...
	code := make([]wasm.Code, run+1)
	code[thrower] = wasm.Code{Body: []byte{wasm.OpcodeI32Const, 0x00, wasm.OpcodeThrow, 0x00, wasm.OpcodeEnd}}
	...
	code[run] = wasm.Code{
		...
		Body: []byte{
			...
			wasm.OpcodeEnd, wasm.OpcodeUnreachable, wasm.OpcodeEnd,
			wasm.OpcodeLocalGet, 0x00, wasm.OpcodeCall, payload,
			wasm.OpcodeEnd,
		},
	}
	m := &wasm.Module{
...
const ehDeepFrameValues = 16

// ehDeepFrame wraps call in a frame that computes ehDeepFrameValues i64s from local p before
// it and uses them after, so that they are spilled across it, and returns what call does.
//
// A popped frame leaves its handles behind below the stack pointer, and the frame of the
// trampoline a collection runs in can land on top of them, where the scan sees them. Making
// an exnref beneath a deep frame keeps those leftovers further down than a trampoline frame
// reaches, so that a test is only passed by finding the handle where it is meant to be.
func ehDeepFrame(p byte, call ...byte) (locals []wasm.ValueType, body []byte) {
	for k := byte(0); k < ehDeepFrameValues; k++ {
		locals = append(locals, wasm.ValueTypeI64)
		body = append(body, wasm.OpcodeLocalGet, p, wasm.OpcodeI64ExtendI32U,
			wasm.OpcodeI64Const, 1+k, wasm.OpcodeI64Add, wasm.OpcodeLocalSet, p+1+k)
	}
	body = append(body, call...)
	body = append(body, wasm.OpcodeI64Const, 0x00)
	for k := byte(0); k < ehDeepFrameValues; k++ {
		body = append(body, wasm.OpcodeLocalGet, p+1+k, wasm.OpcodeI64Add)
	}
	return locals, append(body, wasm.OpcodeI64Eqz, wasm.OpcodeIf, 0x40, wasm.OpcodeUnreachable, wasm.OpcodeEnd, wasm.OpcodeEnd)
}

// ehPayloadBody is the body of a function (param exnref) (result i32) that throws the
// exception again and returns the i32 it carries, caught as tag 0. Throwing it is what
// resolves the handle, so this fails if the exception was let go of.
var ehPayloadBody = []byte{
...
// TestExceptionHandlingCompilerGC covers what the compiler has to release per
...
func TestExceptionHandlingCompilerGC(t *testing.T) {
	if !platform.CompilerSupported() {
		t.Skip()
	}
	const iters = 200_000
...
// TestExceptionHandlingCompilerSnapshotKeepsCollectedExnrefs rewinds to a snapshot whose
// stack holds an exnref that was let go of, and collected, after the snapshot was taken.
// The restored frame uses it, so the snapshot has to have kept it.
func TestExceptionHandlingCompilerSnapshotKeepsCollectedExnrefs(t *testing.T) {
	if !platform.CompilerSupported() {
		t.Skip()
	}
	i32, exnref := wasm.ValueTypeI32, wasm.ValueTypeExnref
	// Functions 0 and 1 are host.snapshot, returning 0 the first time and 1 after the
	// restore, and host.restore, which does not return.
	const (
		thrower = iota + 2
		makeExn
		payload
	...
	code := make([]wasm.Code, run+1-thrower)
	code[thrower-2] = wasm.Code{Body: []byte{wasm.OpcodeI32Const, 0x00, wasm.OpcodeThrow, 0x00, wasm.OpcodeEnd}}
	code[makeExn-2] = wasm.Code{Body: []byte{
		wasm.OpcodeBlock, byte(exnref),
		wasm.OpcodeTryTable, 0x40, 0x01, wasm.CatchKindCatchAllRef, 0x00,
	...
	code[payload-2] = wasm.Code{Body: ehPayloadBody}
	// $capture holds an exnref in a local across the snapshot, and after the restore returns
	// its payload.
	code[capture-2] = wasm.Code{
		LocalTypes: []wasm.ValueType{exnref},
		Body: []byte{
			wasm.OpcodeI32Const, 42, wasm.OpcodeCall, makeExn, wasm.OpcodeLocalSet, 0x00,
			wasm.OpcodeCall, 0x00,
			wasm.OpcodeIf, 0x40,
			wasm.OpcodeLocalGet, 0x00, wasm.OpcodeCall, payload, wasm.OpcodeReturn,
			wasm.OpcodeEnd,
			wasm.OpcodeI32Const, 0x00,
			wasm.OpcodeEnd,
		},
	}
	// $deep runs $capture beneath a deep frame, so that once both return, the local is out of
	// reach of anything that runs from $run.
	code[deep-2].LocalTypes, code[deep-2].Body = ehDeepFrame(0x00, wasm.OpcodeCall, capture)
	// $run returns what $deep does the second time. The first time, it throws, which collects
	// what the call no longer reaches, and rewinds.
	code[run-2] = wasm.Code{Body: []byte{
		wasm.OpcodeI32Const, 0x01, wasm.OpcodeCall, deep,
		wasm.OpcodeLocalTee, 0x00, wasm.OpcodeIf, 0x40,
		wasm.OpcodeLocalGet, 0x00, wasm.OpcodeReturn,
		wasm.OpcodeEnd,
		wasm.OpcodeBlock, 0x40,
		wasm.OpcodeTryTable, 0x40, 0x01, wasm.CatchKindCatchAll, 0x00,
		wasm.OpcodeCall, thrower,
	...
	m := &wasm.Module{
		TypeSection: []wasm.FunctionType{
			{Results: []wasm.ValueType{i32}}, // 0: host.snapshot, $capture
			{},                               // 1: host.restore, $thrower
			{Params: []wasm.ValueType{i32}},  // 2: the tag
			{Params: []wasm.ValueType{i32}, Results: []wasm.ValueType{exnref}}, // 3: $make
			{Params: []wasm.ValueType{exnref}, Results: []wasm.ValueType{i32}}, // 4: $payload
			{Params: []wasm.ValueType{i32}, Results: []wasm.ValueType{i32}},    // 5: $deep
		},
		ImportSection: []wasm.Import{
			{Module: "host", Name: "snapshot", Type: wasm.ExternTypeFunc, DescFunc: 0},
			{Module: "host", Name: "restore", Type: wasm.ExternTypeFunc, DescFunc: 1},
		},
		ImportFunctionCount: 2,
		TagSection:          []wasm.Tag{{Type: 2}},
		FunctionSection:     []wasm.Index{1, 3, 4, 0, 5, 0},
		ExportSection:       []wasm.Export{{Type: wasm.ExternTypeFunc, Name: "run", Index: run}},
		CodeSection:         code,
	}
