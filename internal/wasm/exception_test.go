...
func newExn(s *ExceptionStore, params ...*Exception) *Exception {
	...
	return s.NewException(exnrefTag(len(params)), values, params)
}
...
func TestExceptionStore_unreferencedParamStillTraps(t *testing.T) {
	...
	parent := s.NewException(exnrefTag(1), []uint64{uint64(child.ID)}, nil)
...
func TestExceptionStore_handlesAreTagged(t *testing.T) {
	if unsafe.Sizeof(Reference(0)) < 8 {
		t.Skip("a Reference this narrow has no room for the tag")
	}
	var s ExceptionStore
	require.True(t, MayBeExceptionHandle(uint64(newExn(&s).ID)))

	// What a scan of a stack mostly finds.
	for _, w := range []uint64{0, 1, 42, math.MaxUint32, math.Float64bits(1.5), math.Float64bits(-1e10), math.MaxUint64} {
		require.False(t, MayBeExceptionHandle(w), "%#x", w)
	}
}

func TestExceptionStore_handleSpaceExhausted(t *testing.T) {
	if unsafe.Sizeof(Reference(0)) < 8 {
		t.Skip("a Reference this narrow runs out at its own width")
	}
	var s ExceptionStore
	s.last.Store(1<<exceptionHandleTagShift - 2)

	// The last one the counter has room for under the tag.
	last := newExn(&s)
	require.True(t, MayBeExceptionHandle(uint64(last.ID)))

	err := require.CapturePanic(func() { newExn(&s) })
	require.Equal(t, wasmruntime.ErrRuntimeTooManyExceptions, err)
}
